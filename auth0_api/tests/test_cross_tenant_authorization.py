...
The Auth0 API never touches a database directly: it proxies through the SQL
Query API's governed GraphQL pipeline using the access token held in the
server-side session. These tests prove:

* the API forwards the SESSION's access token, never a client-supplied
  credential (confused-deputy protection),
* the caller-chosen logical database_id is forwarded upstream so the governed
  pipeline resolves it under the principal's org,
* upstream cross-tenant denials surface as errors that leak no row data,
* stale (expired/revoked) sessions and privilege changes fail closed with no
  cached results.

NOTE: the test HTTP client is itself an ``httpx.AsyncClient``, so route-level
tests patch the service class (not ``httpx``); service-level tests may patch
``httpx.AsyncClient.post`` because they call the service directly.
"""
...
# ---------------------------------------------------------------------------
# Route-level forwarding: session credentials only, caller database_id passed through
# ---------------------------------------------------------------------------
class TestTextToSqlRouteConfusedDeputy:
    """The /api/text-to-sql route must hand the SERVICE the session access token,
...
# ---------------------------------------------------------------------------
# Service-level forwarding: Authorization header and databaseId variable
# ---------------------------------------------------------------------------
class TestTextToSqlServiceForwarding:
    """The service forwards the validated session token and database_id upstream."""
    ...
    async def test_service_forwards_session_token_and_database_id(self, mock_ai_service) -> None:
        ...
        with patch(
            "app.services.text_to_sql_service.httpx.AsyncClient.post", return_value=SCHEMA_OK
        ) as mock_post:
            result = await service.generate_sql_from_text(
                "list users", access_token=SESSION_TOKEN, database_id="finance"
            )
    ...
    async def test_service_cross_tenant_denial_leaks_no_rows(self, mock_ai_service) -> None:
        ...
        with patch(
            "app.services.text_to_sql_service.httpx.AsyncClient.post", return_value=denial
        ):
            result = await service.generate_sql_from_text(
                "list users", access_token=SESSION_TOKEN, database_id="finance"
            )
    ...
    async def test_service_execute_round_trip_uses_session_token(self, mock_ai_service) -> None:
        ...
        async def fake_post(url, **kwargs):  # noqa: ANN202 - captures only what tests assert
            posts.append(dict(url=url, **kwargs))
            return SCHEMA_OK if "getTableSchema" in kwargs["json"]["query"] else exec_ok

        with patch("app.services.text_to_sql_service.httpx.AsyncClient.post",
                   new=AsyncMock(side_effect=fake_post)):
            result = await service.generate_sql_from_text(
                "list users", execute=True, access_token=SESSION_TOKEN, database_id="finance"
            )
...
# ---------------------------------------------------------------------------
# B2B GraphQL proxy: server-side credentials only, CSRF-gated
# ---------------------------------------------------------------------------
class TestGraphqlProxyConfusedDeputy:
    """The frontend proxy must never forward a client-supplied credential."""

    @pytest.mark.asyncio
    async def test_proxy_forwards_session_token_ignoring_client_authorization(
        self, client, mocker
    ) -> None:
...
# ---------------------------------------------------------------------------
# Stale sessions: expired TTL rejected; revoked upstream token yields no data
# ---------------------------------------------------------------------------
class TestStaleSessions:
    """Expired or revoked session credentials must never serve cached results."""
    ...
    async def test_revoked_upstream_token_yields_error_not_data(self, mock_ai_service) -> None:
        ...
        with patch(
            "app.services.text_to_sql_service.httpx.AsyncClient.post", return_value=_http_401()
        ):
            result = await service.generate_sql_from_text(
                "list users", access_token="revoked-token", database_id="default"
            )
...
# ---------------------------------------------------------------------------
# Privilege changes are enforced per request, never cached
# ---------------------------------------------------------------------------
class TestPrivilegeChanges:
    """A role downgrade mid-session takes effect on the very next request."""

    @pytest.mark.asyncio
    async def test_admin_overview_role_revocation_takes_effect_next_request(
